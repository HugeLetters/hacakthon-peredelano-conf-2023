<script lang="ts">
	...
	const reportQuery = data.trpc.report.getUserReport.query({ reportId: $page.params.id });
...
<div class="wrapper">
	{#if $reportQuery.isSuccess}
		...
		<div class="reportData">
			<span>
				<svelte:component this={iconByCategory($reportQuery.data?.category)} />
			</span>
			<span>{$reportQuery.data?.country}</span>
			<span>{$reportQuery.data?.createdAt}</span>
		</div>
		...
		<div class="content">
			...
			<h4>Организация</h4>
			<p class="text">{$reportQuery.data?.organization}</p>
		</div>

		<a href="/case/{$reportQuery.data?.caseId}/report/{$reportQuery.data?.id}" class="chatLink">
			<svg
				width="36"
				height="36"
				viewBox="0 0 36 36"
